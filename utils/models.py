...
Staff:
{''.join([f"{staff.department} - {staff.position}: {staff.employee.first_name} {staff.employee.last_name}{nl}" for staff in item.job_set.all().order_by('department')])}
        """  # noqa
...
Staff:
{''.join([f"{staff.department} - {staff.position}: {staff.employee.first_name} {staff.employee.last_name}{nl}" for staff in item.job_set.filter(department=self.department[0]).order_by('department')])}
        """  # noqa
